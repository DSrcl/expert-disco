...
/* CS 526 stuff */
MachineInstr *X86InstrInfo::getGlobalPICAddr(MachineFunction &MF, unsigned DestReg,
  ...
  MachineInstrBuilder LoadMI = BuildMI(MF, DebugLoc(), get(Opc), DestReg);
...
MachineInstr *X86InstrInfo::storeReg(MachineFunction &MF, unsigned SrcReg,
  ...
  auto Store = BuildMI(MF, DebugLoc(), get(Opc));
  addFullAddress(Store, AM);
  Store.addReg(SrcReg);
  return Store;
}
/* end of CS 526 stuff */
